...
class SpladeTrainer(TevatronTrainer):
    def __init__(self, *args, **kwargs):
        ...
        # TODO: investigate whether negatives_x_device argument affects the performance

    @staticmethod
    def _flops(inputs):
        return torch.sum(torch.mean(torch.abs(inputs), dim=0) ** 2)

    def compute_loss(self, model: PreTrainedModel, inputs: torch.Tensor):
...
def main():
    ...
    model = SpladeModel.build(
        ...
        training_args,
        config=config,
        trust_remote_code=True,  # for modern bert structure
        cache_dir=model_args.cache_dir,
    )
