...
class ModelArguments:
    ...
    lora_r: int = field(default=64, metadata={"help": "rank of lora"})
    lora_alpha: int = field(default=128, metadata={"help": "alpha of lora"})
        ...
        if self.train_dir is not None:
            if os.path.isdir(self.train_dir):
                ...
                self.train_dir = os.path.join(
                    os.path.abspath(os.getcwd()), self.train_dir
                )
                self.train_path = [
...
class TevatronTrainingArguments(TrainingArguments):
    ...
    lr_scheduler_type: str = field(
        ...
        metadata={
            "help": "choose from following options: constant_with_warmup, linear, cosine"
        },
    )
    ...
    optim: str = field(default="adamw_hf", metadata={"help": "optimizer of choice"})
    ...
    save_only_model: bool = field(
        default=True,
        metadata={
            "help": "save only model or save optimizer state and scheduler state as well"
        },
    )
    flops_warmup: int = field(
        default=10000, metadata={"help": "number of warmup steps"}
    )
