...
from .preprocessor import EchoTrainPreProcessor, EchoQueryPreProcessor, EchoCorpusPreProcessor
from ..arguments import DataArguments

DEFAULT_PROCESSORS = [TrainPreProcessor, QueryPreProcessor, CorpusPreProcessor]
DEFAULT_ECHO_PROCESSORS = [EchoTrainPreProcessor, EchoQueryPreProcessor, EchoCorpusPreProcessor]
...
PROCESSOR_INFO = {
    ...
    'Tevatron/msmarco-passage': DEFAULT_ECHO_PROCESSORS,  # change to echo processors 
    'Tevatron/beir': DEFAULT_ECHO_PROCESSORS,
    'brutusxu/msmarco-passages-aug-small': DEFAULT_ECHO_PROCESSORS,
    'Tevatron/msmarco-passage-aug': DEFAULT_ECHO_PROCESSORS,
}

HF_TOKEN = os.environ.get('HF_TOKEN') or os.environ.get('HUGGING_FACE_HUB_TOKEN')
if not HF_TOKEN:
    raise Warning(f"No HuggingFace token found in environment variables (HF_TOKEN or HUGGING_FACE_HUB_TOKEN). Some models may not be accessible.")
...
class HFTrainDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        ...
        self.dataset = load_dataset(data_args.dataset_name,
                                    ...
                                    cache_dir=cache_dir, 
                                    token=HF_TOKEN,
                                    trust_remote_code=True)[data_args.dataset_split]
