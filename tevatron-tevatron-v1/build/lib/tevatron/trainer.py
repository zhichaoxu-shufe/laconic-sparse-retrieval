...
class TevatronTrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        _is_torch_generator_available = (
            True if version.parse(torch.__version__) >= version.parse("1.6") else False
        )
    ...
    def compute_loss(self, model, inputs) -> torch.Tensor:
...
class TevatronSFTTrainer(SFTTrainer):
    def __init__(self, *args, **kwargs):
        super(TevatronSFTTrainer, self).__init__(*args, **kwargs)
        self._dist_loss_scale_factor = (
            dist.get_world_size() if self.args.negatives_x_device else 1
        )
    ...
    def training_step(self, *args):
        return (
            super(TevatronSFTTrainer, self).training_step(*args)
            / self._dist_loss_scale_factor
        )
